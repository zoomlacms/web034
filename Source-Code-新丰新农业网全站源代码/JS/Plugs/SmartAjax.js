(function (window) {
    /*
      1,发起申请-->无缓存,则发起成功后写入,如果失败,则隔一定时间重新发起
    ...
    */
    ...
    //支持local持久化存储机制
    //支持过期时间
    //ex_store: true,ex_refresh: false,
    var SmartAjax = {
        ajax: function (opts) {
            var ref = this;
            ...
            opts.success = function (data, textStatus) {
                //未设为不缓存,并且获取成功,则加入缓存
                if (opts.ex_store !== false && !ZLCache.isEmpty(data) && data.retcode == 1) {
            ...
            for (var i = 0; i < localStorage.length; i++) {
                ...
                try {
                    ...
                    if (model.date) {
                        ...
                        if (second > AjaxConfig.timeout) { ref.remove(key); }
                        SmartAjax.log(key, "清除超时成功");
                    }
