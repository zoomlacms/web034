<%@ page language="C#" masterpagefile="~/Manage/I/Default.master" autoeventwireup="true" inherits="manage_AddOn_Maps, App_Web_d5whfeq0" clientidmode="Static" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <ZL:ExGridView runat="server" ID="EGV" AutoGenerateColumns="false" AllowPaging="true" PageSize="10" EnableTheming="False" IsHoldState="false"
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="操作">
                    ...
                    <ItemTemplate>
                        <a onclick="aa(<%# Eval("mid")%>)" title="修改" class="option_style"><i class="fa fa-pencil" title="修改"></i></a>
                        <a href="javascript:if(confirm('你确定要删除吗?')) window.location.href='Maps.aspx?AUId=<%# Eval("mid")%>';" title="删除" class="option_style"><i class="fa fa-trash-o" title="删除"></i></a>
                        <asp:LinkButton ID="LinkButton8" runat="server" CommandName="ifframe" CommandArgument='<%# Eval("mid") %>' ToolTip="获取地图代码" CssClass="option_style"><i class="fa fa-font" title="获取代码"></i>获取地图代码</asp:LinkButton>
                    </ItemTemplate>
                    <ItemStyle />
                </asp:TemplateField>
            </Columns>
