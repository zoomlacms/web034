<%@ page language="C#" masterpagefile="~/Manage/I/Default.master" autoeventwireup="true" inherits="Manage_I_Content_ManageJsContent, App_Web_4l5orxfu" clientidmode="Static" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
                ...
                <tr onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'" class="tdbg" ondblclick="getinfo('<%#Eval("id")%>','<%#Eval("JsType")%>')">
                    ...
                    <td align="center">
                        <a href="<%#(Eval("JsType","{0}")=="1")?"AddJsPic.aspx":"AddJsContent.aspx" %>?menu=edit&id=<%#Eval("id") %>" class="option_style"><i class="fa fa-gear" title="设置"></i>参数设置</a> <a href="?menu=refresh&id=<%#Eval("id") %>" class="option_style"><i class="fa fa-refresh" title="刷新"></i>刷新</a>
                         <a href="?menu=delete&id=<%#Eval("id") %>" onclick="return confirm('真的要删除此JS文件吗？如果有文件或模板中使用此JS文件，请注意修改过来！');" class="option_style"><i class="fa fa-trash-o" title="删除"></i>删除</a>
                    </td>
                </tr>
            </ItemTemplate>
        </asp:Repeater>
