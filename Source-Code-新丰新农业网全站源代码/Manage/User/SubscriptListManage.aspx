<%@ page language="C#" autoeventwireup="true" inherits="manage_User_SubscriptListManage, App_Web_wopv4rdr" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
                ...
                OnPageIndexChanging="EGV_PageIndexChanging" OnRowCommand="EGV_RowCommand" OnRowDataBound="GridView1_RowDataBound"> 
                        <Columns>
                            ...
                            <asp:TemplateField HeaderText="操作">
                                <ItemTemplate>
                                    <asp:LinkButton ID="upd" runat="server" PostBackUrl='<%# Eval("Id","AddSubscriptionCount.aspx?menu=update&id={0}") %>' CssClass="option_style"><i class="fa fa-pencil" title="修改"></i></asp:LinkButton>
                                    <asp:LinkButton ID="Del" runat="server" CommandName="Del" CommandArgument='<%#Eval("Id") %>' OnClientClick="if(confirm('确定删除?')) return true; else return false;" CssClass="option_style"><i class="fa fa-trash-o" title="删除"></i></asp:LinkButton>
                                    <asp:LinkButton ID="end" runat="server" CommandName="End" CommandArgument='<%#Eval("Id") %>' CssClass="option_style"><i class="fa fa-ban" title="终止"></i>终止订阅/</asp:LinkButton>
                                    <asp:LinkButton ID="audit" runat="server" CommandName="Audit" CommandArgument='<%#Eval("Id") %>' CssClass="option_style"><i class="fa fa-legal" title="审核"></i>通过审核</asp:LinkButton>
                                </ItemTemplate>
                                <ItemStyle HorizontalAlign="Center" />
                            </asp:TemplateField>
                        </Columns>
