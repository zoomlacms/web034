<%@ page language="C#" autoeventwireup="true" inherits="manage_User_SystemUserModel, App_Web_wopv4rdr" enableviewstatemac="false" masterpagefile="~/Manage/I/Default.master" enableEventValidation="false" viewStateEncryptionMode="Never" %>
    ...
    <asp:Repeater ID="RepModelField" runat="server" OnItemCommand="Repeater1_ItemCommand">
      <ItemTemplate>
          <tr style="text-align: center;">
              ...
              <td align="center"><%# GetStyleTrue(Eval("IsNotNull", "{0}"))%></td>
              <td align="center">
                  <asp:LinkButton ID="LinkButton2" runat="server" CommandName="UpMove" CommandArgument='<%# Eval("FieldID") %>' CssClass="option_style"><i class="fa fa-arrow-up" title="上移"></i>上移</asp:LinkButton>
                  <asp:LinkButton ID="LinkButton3" runat="server" CommandName="DownMove" CommandArgument='<%# Eval("FieldID") %>' CssClass="option_style">下移<i class="fa fa-arrow-down" title="下移"></i></asp:LinkButton></td>
              <td align="center">
                  <a href="../Content/AddModelField.aspx?ModelID=-1&ModelType=9&FieldID=<%# Eval("FieldID") %>" class="option_style"><i class="fa fa-pencil" title="修改"></i></a>
                  <asp:LinkButton ID="LinkButton1" runat="server" CommandName="Delete" CommandArgument='<%# Eval("FieldID") %>' OnClientClick="return confirm('确定删除此字段吗?')" CssClass="option_style"><i class="fa fa-trash-o" title="删除"></i>删除</asp:LinkButton></td>
          </tr>
      </ItemTemplate>
    </asp:Repeater>
  </table>
